...
class AnalyzeView(APIView):
    def post(self, request, format=None):
        ...
        if isinstance(data, dict) and "tasks" in data:
            ...
            return Response(
                {"detail": "Invalid payload. Send a JSON list of tasks or {'tasks': [...]}."},
        ...
        if not serializer.is_valid():
            return Response(
                {"detail": "Invalid task data", "errors": serializer.errors},
                status=status.HTTP_400_BAD_REQUEST
            )
...
def suggest_view(request):
    ...
    if not tasks_param:
        return Response(
            {"detail": "Provide tasks via ?tasks=[JSON] or POST to /api/tasks/analyze/"},
            status=400
        )
    ...
    return Response(
        {"strategy": strategy, "suggestions": top3, "cycles": result["cycles"], "warnings": result["warnings"]},
        status=200
    )
